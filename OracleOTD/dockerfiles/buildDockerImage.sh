...
#
# Purpose: Builds an Oracle Traffic Director Docker Image
#
...
Builds a Docker Image for Oracle Traffic Director
Parameters:
   -h: view usage
...
checkFilePackages() {
  ...
  jarList=`grep -v -e "^#.*" install/otd.download | awk '{print $2}'`
...
checksumPackages() {
  ...
  md5sum --quiet -c install/otd.download 2> /dev/null
  if [ "$?" -ne 0 ]; then
    cat <<EOF
...
if [ ${VERSION} = 12.2.1.3 ]
then
  IMAGE_NAME="${DC_REGISTRY_OTD}/oracle/otd:$VERSION"
...
if [ $? -eq 0 ]; then
  cat << EOF
INFO: Oracle Traffic Director Docker Image for version: $VERSION 
...
EOF
  ...
  echo "ERROR: Oracle Traffic Director Docker Image was NOT successfully created. Check the output and correct any reported problems with the docker build operation."
fi
