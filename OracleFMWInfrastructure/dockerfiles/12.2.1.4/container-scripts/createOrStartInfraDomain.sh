...
RUN_RCU="true"
CONTAINERCONFIG_DIR=/opt/oracle/ContainerData
...
echo -e $DB_PASSWORD"\n"$DB_SCHEMA_PASSWORD > /opt/oracle/pwd.txt
...
then
   ...
   then
       ...
       then
           ...
           grep -q "RCU-6016 The specified prefix already exists" "/opt/oracle/RCU.out"
           if [ $? -eq 0 ] ; then
       ...
       rm -f "/opt/oracle/pwd.txt" 
   fi

        echo "Domain Configuration Phase"
        echo "=========================="
    
        wlst.sh -skipWLSModuleScanning /opt/oracle/container-scripts/createInfraDomain.py -oh $ORACLE_HOME -jh $JAVA_HOME -parent $DOMAIN_ROOT -name $DOMAIN_NAME -user $ADMIN_USER -password $ADMIN_PASSWORD -rcuDb $CONNECTION_STRING -rcuPrefix $RCUPREFIX -rcuSchemaPwd $DB_SCHEMA_PASSWORD
        retval=$?
